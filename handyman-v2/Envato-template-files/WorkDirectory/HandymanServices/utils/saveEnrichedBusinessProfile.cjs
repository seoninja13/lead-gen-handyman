/**
 * Utility function to save enriched business profile data to Supabase (CommonJS version)
 */
 ...
 */
async function saveEnrichedBusinessProfile(enrichedBusinessProfile) {
  try {
    ...
    const { data: existingProfiles, error: queryError } = await supabase
      .from('test-delete')
      .select('id, city, business, service')
      .eq('business', name)
      .limit(1);
    ...
    if (existingProfiles && existingProfiles.length > 0) {
      ...
      console.log(`Updated enriched business profile for "${name}" in test-delete table`);
    } 
    // Otherwise, insert a new record
      ...
      console.log(`Inserted new enriched business profile for "${name}" in test-delete table`);
    }
 ...
 */
async function getEnrichedBusinessProfiles(options = {}) {
  try {
    const {
      limit = 10,
      offset = 0
 ...
 */
async function getEnrichedBusinessProfileById(id) {
  try {
    const { data, error } = await supabase
      .from('test-delete')
      .select('*')
