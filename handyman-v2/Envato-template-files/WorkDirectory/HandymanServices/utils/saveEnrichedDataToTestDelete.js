/**
 * Utility to save enriched data to the test-delete table
 */
...
/**
 * Save enriched data to the test-delete table
 * 
 * @param {Object} businessName - The business name
...
async function saveEnrichedData(businessName, enrichedData) {
  try {
    ...
    if (existingRecords && existingRecords.length > 0) {
      ...
      console.log(`Updated enriched data for "${businessName}"`);
    } 
    // Otherwise, insert a new record
      ...
      console.log(`Inserted new enriched data for "${businessName}"`);
    }
 ...
 */
async function getEnrichedData(businessName) {
  try {
    ...
    console.error('Error in getEnrichedData:', error);
    throw error;
  }
}
