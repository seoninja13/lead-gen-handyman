...
/**
 * Check if the businesses table has an enriched_data column and add it if it doesn't
 * @returns {Promise<boolean>} - Whether the column exists or was added successfully
 */
async function ensureEnrichedDataColumn() {
  try {
    console.log('Checking if businesses table has enriched_data column...');
    
    // Get a sample record to check the table structure
    ...
    if (data && data.length > 0) {
      ...
      console.log('enriched_data column does not exist. Will try to add it when saving data.');
    } else {
      console.log('Businesses table is empty. Will create enriched_data column when saving data.');
    }
...
async function saveEnrichedDataToBusiness(businessName, city, service, enrichedData) {
  try {
    ...
    if (queryError && !queryError.message.includes('column "business_name" does not exist')) {
      console.error('Error checking for existing business:', queryError);
      throw queryError;
    }
    
    // Prepare the record to insert or update
    // Try different column combinations to handle different table structures
    const record = {};
    ...
    // Try different column names for the city
    record.city = city;
    record.location = city;
    
    // Try different column names for the service
    ...
    // Also save as a string in the description field
...
async function main() {
  try {
    ...
    if (!columnExists) {
      console.error('Failed to ensure enriched_data column exists. Continuing anyway...');
    }
    
    // Get business data to enrich
    // For this example, we'll use a few sample businesses
    const businesses = [
      { name: 'Sacramento Handyman Services', city: 'Sacramento', service: 'Handyman' },
      { name: 'Sacramento Water Damage Pros', city: 'Sacramento', service: 'Water Damage Restoration' }
    ];
