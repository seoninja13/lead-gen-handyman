/**
 * Script to test saving enriched data in the specified format
 */

const { saveEnrichedDataFormat } = require('./utils/saveEnrichedDataFormat');
...
async function testSaveFormat() {
  try {
    console.log('Testing saving enriched data in the specified format...');
    
    const result = await saveEnrichedDataFormat(businessName, enrichedData);
    
    console.log('Enriched data saved successfully in the specified format!');
    console.log('Result:', result.updated ? 'Updated existing record' : 'Inserted new record');
  } catch (error) {
    console.error('Error testing save enriched data format:', error);
  }
}
